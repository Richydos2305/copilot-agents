...
```
editors/
  agents/<name>/agent.md      — persistent personas, selected from the agent picker
  prompts/<name>/prompt.md    — single-purpose slash commands
  shared/<name>.md            — reference content loaded on demand by agents/prompts
```
...
Copy this folder's contents into your VS Code user prompts folder, preserving the `agents/`, `prompts/`, and `shared/` structure:

```bash
cp -r editors/. "/Users/$(whoami)/Library/Application Support/Code/User/prompts/"
```

> **macOS only.** On Windows the path is `%APPDATA%\Code\User\prompts\`. On Linux: `~/.config/Code/User/prompts/`.

> **Note:** this assumes your Copilot setup discovers `.agent.md`/`.prompt.md` files recursively under the prompts folder. If your version only scans the top level, you'll need to flatten the copy instead.
...
| **Shared** | `shared/<name>.md` | Shared reference content. Loaded on demand via markdown links from agents/prompts. Single source of truth. |

---

## Agents

### Backend Dev (`agents/backend-dev/agent.md`)
...
### Frontend Dev (`agents/frontend-dev/agent.md`)
...
### GitHub Expert (`agents/github/agent.md`)
...
### LinkedIn (`agents/linkedin/agent.md`)
...
### Bug Reporter (`agents/bug-reporter/agent.md`)
...
### Issue Fixer (`agents/fix-issues/agent.md`)
...
Junior-friendly, explanation-heavy PR reviewer. Reviews section by section, checks security/performance/architecture/style, and concludes with an Approve or Request Changes verdict. Never applies fixes.

### Test Writer (`agents/write-tests/agent.md`)
...
### `shared/pr-description.md`
Generates a PR description from a diff using the WHY/WHAT/HOW/THINGS-TO-NOTE template.

### `shared/raise-pr.md`
Title derivation, base branch confirmation, and `gh pr create`, used by the GitHub Expert agent.
...
These files are created automatically and live locally only (gitignored), at the root of the deployed prompts folder — not part of the `agents/`/`prompts/`/`shared/` structure:
